...
# class HierarchicalPlan(action_spec_ignored.ActionSpecIgnored):
#     """
#     Component representing the agent's high-level and situational plans,
#     as well as suggests a next move.
#     """
...
#         clock_now=None,
#         pre_act_key: str = "Hierarchical plan",
#         logging_channel: logging.LoggingChannel = logging.NoOpLoggingChannel,
#     ):
...
class HierarchicalPlan(
    ...
    def pre_act(
        ...
        hard_facts, answers, relevant_memories = self._information.generate_information(call_to_action)
        information_str = (
        ...
        relevant_memories_str = (
            f"Relevant memories of {agent_name}:\n"
            f"{relevant_memories}"
            "\n\n"
        )
        if self._goal:
        ...
        # HIGH-LEVEL PLAN:
        high_level_prompt = interactive_document.InteractiveDocument(self._model)
        high_level_prompt.statement(relevant_memories_str)
        high_level_prompt.statement(information_str)
        if self._goal:
            high_level_prompt.statement(goal_str)
        high_level_prompt.statement(
            f"{agent_name}'s current high-level plan:\n{self._high_level_plan}"#
        )
        high_level_prompt.statement(observation)
        if self._clock is not None:
            high_level_prompt.statement(
                f"Current time: {self._clock.current_time_interval_str()}.\n\n"
            )
        ...
        should_replan = high_level_prompt.yes_no_question(question)
        if should_replan or not self._high_level_plan:
            #tom_prompt = "."
            # if self._theory_of_mind:
            #     tom_prompt = (
            #         ", paying particular attention to the predicted "
            ...
            question = (
                "Given the above, write a high-level plan for "
                f"{agent_name} that can span multiple days. It should "
                f"far sighted and give {agent_name} the best possible chance "
            ...
            relevant_principles = (
                self._principle_shards.get_relevant_principles(
                    _get_class_name(self),
                    f"{high_level_prompt.view().text()}\n" f"Excercise: {question}",
                )
            )
            high_level_prompt.statement(
                f"Relevant principles of {agent_name}: {relevant_principles}"
                "\n\n"
            )
            plan = high_level_prompt.open_question(
                question,
                max_tokens=2000,
                terminators=(),
                question_label="Exercise",
            ...
            plan_start = plan.rfind("High-level Plan:") + 16
            self._high_level_plan = plan[plan_start:]
        self._log.update(
            {
                "High Level Plan": self._high_level_plan,
                "High Level Plan CoT": high_level_prompt.view().text().splitlines(),
            }
        )
        # SUGGESTED NEXT ACTION
        action_prompt = interactive_document.InteractiveDocument(self._model)
        action_prompt.statement(information_str)
        if self._goal:
            action_prompt.statement(goal_str)
        action_prompt.statement(
            f"{agent_name}'s current high-level plan:\n{self._high_level_plan}"#
        )
        action_prompt.statement(observation)
        if self._clock is not None:
            action_prompt.statement(
                f"Current time: {self._clock.current_time_interval_str()}.\n\n"
            )
        ...
        action_prompt.statement(call_to_action_str)
        action_prompt.statement("\n\n")
        ...
        self._log.update(
            {
                ...
                "Suggested Action CoT": action_prompt.view().text().splitlines(),
            }
        )
        self._logging_channel(self._log)
        self._log = {"Key": self._pre_act_key}
