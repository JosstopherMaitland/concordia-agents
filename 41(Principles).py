...
class PeopleManager(agent_components.action_spec_ignored.ActionSpecIgnored):
    """
...
class PrincipleShards(action_spec_ignored.ActionSpecIgnored):
    ...
    def __init__(
        ...
        self._logs = {"Key": pre_act_key}

    def _make_pre_act_value(self):

        self._logging_channel(self._logs)
...
class HardFacts(action_spec_ignored.ActionSpecIgnored):
    """
    Component which gleans hard facts about the world after the first set of
    observations.
    """
    ...
    def _make_pre_act_value(self) -> str:
        ...
        if self._hard_facts is None:
            hard_facts_prompt = interactive_document.InteractiveDocument(
                self._model
            )
            mems = "\n".join([mem.text for mem in _get_all_memories(memory)])
            hard_facts_prompt.statement(f"{mems}")
            self._hard_facts = hard_facts_prompt.open_question(
                question=(
                    "Thoroughly analyse the above to determine the hard facts "
                    f"about the external world that {agent_name} is in. "
                    "Facts should pertain to the world, and not only apply to "
                    "the current situation. Only list these facts."
                ),
                max_tokens=1000,
                terminators=(),
                question_label="Exercise",
            ).rstrip("\n")
            self._log.update({"CoT": hard_facts_prompt.view().text().splitlines()})
        self._log.update(
            {
                "Hard Facts": self._hard_facts,
            }
        )
        self._logging_channel(self._log)
        self._log = {"Key": self.get_pre_act_key()}
        return self._hard_facts
...
class SituationRepresentation(action_spec_ignored.ActionSpecIgnored):
    ...
    def __init__(
        ...
        hard_facts_comp_name: entity_component.ComponentName,
        memory_component_name: str = (
            memory_component.DEFAULT_MEMORY_COMPONENT_NAME
        ),
        ...
        self._clock_now = clock_now
        self._memory_component_name = memory_component_name
        self._hard_facts_comp_name = hard_facts_comp_name
        self._logging_channel = logging_channel
    ...
    def _make_pre_act_value(self) -> str:
        ...
        hard_facts = self.get_named_component_pre_act_value(
            self._hard_facts_comp_name
        )
        memory = self.get_entity().get_component(
            self._memory_component_name, type_=memory_component.MemoryComponent
        )
        ...
        interval_scorer = legacy_associative_memory.RetrieveTimeInterval(
            time_from=self._previous_time - datetime.timedelta(hours=1),
        ...
        if situation_change:
            ...
            self._log.update({"Current Situation CoT": current_situation_prompt.view()
                .text()
                .splitlines()})
        self._log.update(
            {
...
def build_agent(
    ...
    hard_facts_label = "Hard facts"
    hard_facts = HardFacts(
        model=model,
        pre_act_key=hard_facts_label,
        logging_channel=measurements.get_channel(hard_facts_label).on_next,
    )
    ...
    current_situation = SituationRepresentation(
        model=model,
        clock_now=clock.now,
        hard_facts_comp_name=_get_class_name(hard_facts),
    ...
    principles_label = f"Principles of {agent_name}"
    principles = PrincipleShards(
        model=model,
        principles=(
        ...
        pre_act_key=principles_label,
        logging_channel=measurements.get_channel(principles_label).on_next,
    )
