...
class PeopleManagerNoPredictions(action_spec_ignored.ActionSpecIgnored):
    """
    ...
    def gather_information(self, person: str) -> str:
        ...
        model_interface = interactive_document.InteractiveDocument(self._model)
        model_interface.statement(f"Current date and time: {self._clock.current_time_interval_str()}.\n")
        model_interface.statement(
        ...
        question = (
            "Infer and state the goals and preferences of {person}."
        )
    ...
    def get_relevant(self) -> Sequence[str]:
        current_situation =
