...
class PeopleManager(agent_components.action_spec_ignored.ActionSpecIgnored):
    ...
    def make_predictions(self, person: str) -> str:
        ...
        self._logging_dict.update(
            {
                f"Prediction Chain of Thought for {person}": model_interface
                .view()
                .text()
                .splitlines()
            }
...
class PrincipleShards(action_spec_ignored.ActionSpecIgnored):
    ...
    def get_relevant_principles(
        self,
        component_name: entity_component.ComponentName,
        task:str
    ) -> str:
        agent_name = self.get_entity().name
    ...
    def _make_pre_act_value(self) -> str:
        ...
        if self._situation_thus_far is None:
            ...
            chain_of_thought.statement(
                f"Story fragments and world data:\n{mems}"
            )
            chain_of_thought.statement(f"Events continue after {current_time}")
            self._situation_thus_far = chain_of_thought.open_question(
                question=(
                    "Narratively summarize the story fragments and world "
                    "data. Give "
                    "special emphasis to atypical features of the setting "
                    "such as "
                    "when and where the story takes place as well as any "
                    "causal "
                    "mechanisms or affordances mentioned in the information "
                    "provided. Highlight the goals, personalities, "
                    "occupations, "
        ...
        self._situation_thus_far = chain_of_thought.open_question(
            question=(
                ...
                "actions may produce, and what is currently going on. Make "
                "sure to look at timestamps to ensure the description of "
                "the current situation is accurate."
            ),
            max_tokens=1000,
            terminators=(),
            question_label="Exercise",
